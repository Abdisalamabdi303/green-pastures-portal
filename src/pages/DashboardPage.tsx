...
const DashboardPage = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      setLoading(true);
      const [animalsResponse, expenses] = await Promise.all([
        animalServices.getAnimals(1, 1000), // Get all animals for dashboard
        expenseServices.getExpenses()
      ]) as [{ animals: Animal[]; total: number }, Expense[]];

      const animals = animalsResponse.animals;

      // Calculate statistics
      const totalAnimals = animalsResponse.total;
